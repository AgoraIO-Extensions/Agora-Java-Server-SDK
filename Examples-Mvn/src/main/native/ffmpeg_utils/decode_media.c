...
int resample_audio(DecodeContext *decode_ctx, AVFrame *frame) {
    ...
    if (swr_ctx) {
        ...
        //   dst_nb_samples, frame->nb_samples); return -1;
        // }
    }
    decode_ctx->dst_nb_samples = dst_nb_samples;
    int buf_size = av_samples_get_buffer_size(NULL, decode_ctx->dst_ch_layout.nb_channels,
                                              dst_nb_samples, decode_ctx->dst_sample_fmt, 1);
    if (decode_ctx->buffer == NULL || decode_ctx->buffer_size < buf_size) {
        ...
        av_samples_fill_arrays(decode_ctx->samples, NULL, decode_ctx->buffer,
                               decode_ctx->dst_ch_layout.nb_channels, dst_nb_samples,
                               decode_ctx->dst_sample_fmt, 1);
    }
    decode_ctx->actual_buffer_size = buf_size;
    if (!swr_ctx) {
        // just copy audio data
        result = av_samples_copy(decode_ctx->samples, frame->data, 0, 0, frame->nb_samples,
                                 codec_ctx->ch_layout.nb_channels, codec_ctx->sample_fmt);
        if (result < 0) {
    ...
    // resample audio data
    result = swr_convert(swr_ctx, decode_ctx->samples, dst_nb_samples,
...
int init_sws(DecodeContext *decode_ctx) {
    ...
    int ret = av_image_alloc(decode_ctx->dst_data, decode_ctx->dst_linesize, decode_ctx->dst_width,
                             decode_ctx->dst_height, decode_ctx->dst_pix_fmt, 1);
    if (ret < 0) {
        av_log(NULL, AV_LOG_ERROR, "Can't allocate image\n");
        return ret;
    }
    decode_ctx->buffer = decode_ctx->dst_data[0];
    decode_ctx->buffer_size = av_image_get_buffer_size(
        decode_ctx->dst_pix_fmt, decode_ctx->dst_width, decode_ctx->dst_height, 1);

    if (decode_ctx->dst_pix_fmt == codec_ctx->pix_fmt &&
        decode_ctx->dst_width == codec_ctx->width && decode_ctx->dst_height == codec_ctx->height) {
        return 0;
    }
    struct SwsContext *sws_ctx = sws_getContext(
        codec_ctx->width, codec_ctx->height, codec_ctx->pix_fmt, decode_ctx->dst_width,
        decode_ctx->dst_height, decode_ctx->dst_pix_fmt, SWS_BILINEAR, NULL, NULL, NULL);
    if (!sws_ctx) {
...
int resize_video(DecodeContext *decode_ctx, AVFrame *fr) {
    ...
    if (!sws_ctx) {
        int ret = av_image_copy_to_buffer(
            decode_ctx->buffer, decode_ctx->buffer_size, (const uint8_t *const *)fr->data,
            (const int *)fr->linesize, ctx->pix_fmt, ctx->width, ctx->height, 1);
        if (ret < 0) {
            av_log(NULL, AV_LOG_ERROR, "Error copying image to buffer\n");
        }
        return ret;
    }
    int ret = sws_scale(sws_ctx, (const uint8_t *const *)fr->data, fr->linesize, 0, ctx->height,
                        (uint8_t *const *)decode_ctx->dst_data, decode_ctx->dst_linesize);
    if (ret < 0) {
...
int init_decoder(MediaDecoder *decoder, int media_type) {
    ...
    if (media_type == AVMEDIA_TYPE_VIDEO) {
        av_log(NULL, AV_LOG_INFO,
               "stream index %d, video codec: %s, pix_fmt %s, width %d, height %d\n",
               decode_ctx->stream_index, codec->name, av_get_pix_fmt_name(codec_ctx->pix_fmt),
               codec_ctx->width, codec_ctx->height);
        init_sws(decode_ctx);
    } else if (media_type == AVMEDIA_TYPE_AUDIO) {
        av_log(NULL, AV_LOG_INFO,
               "stream index %d, audio codec: %s, sample_fmt %s, sample_rate %d, channels %d, "
               "frame_size %d\n",
...
void *open_media_file(const char *file_name) {
    ...
    av_log(NULL, AV_LOG_DEBUG, "open_media_file file name: %s\n", file_name);
    ...
    if (d->fmt_ctx->duration != AV_NOPTS_VALUE) {
        ...
        for (int i = 0; i < d->fmt_ctx->nb_streams; i++) {
            ...
            if (stream->duration != AV_NOPTS_VALUE) {
                ...
                if (stream_duration > d->duration) {
                    d->duration = stream_duration / 1000;
                }
            }
        }
    }
