...
std::unique_ptr<HelperAudioFrame> HelperAacFileParser::getAudioFrame(int frameSizeDuration) {
    ...
    if (aacframe.syncword != 0xfff) {
        printf("Invalid AAC syncword 0x%x", aacframe.syncword);
        return audioFrame;
    }
    ...
    int sampleRateHz = AacFrameSampleRateMap[aacframe.sampling_frequency_index];
    int codec = 8;  // agora::rtc::AUDIO_CODEC_AACLC;
    // calculate audio frame size per channel
    int samplesPerChannel = sampleRateHz * frameSizeDuration / 1000;

    // ownsBuffer=false because buffer points to data_buffer_ (internal buffer, not allocated with
    // new[])
    audioFrame.reset(new HelperAudioFrame(numberOfChannels, sampleRateHz, codec, samplesPerChannel,
                                          &data_buffer_[data_offset_], aacframe.aac_frame_length,
                                          false));
